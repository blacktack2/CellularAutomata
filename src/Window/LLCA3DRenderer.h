...
class LLCA3DRenderer : public Renderer {
public:
	LLCA3DRenderer(Window& window, GLuint& texture);
	~LLCA3DRenderer();
	...
	inline glm::uvec3 getBounds() { return mSimulator.getBounds(); }
	inline void setBounds(glm::uvec3 bounds) {
		mSimulator.setBounds(bounds);
		mShader.bind();
		glUniform3uiv(mSimBoundsUniform, 1, &bounds[0]);
		mShader.unbind();
	}

	inline void setNumGenerations(LLCA3DSimulator::cell numGenerations) {
